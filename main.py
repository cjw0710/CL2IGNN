...
# adj, features, labels = data_load.load_data_dgl(dataset_name)
...
if args.dataset == 'cora':
    ...
    adj, features, labels = data_load.load_wiki_cs()
    im_class_num = 10
    class_sample_num = 20 #not used    
else:
...
decoder = models.Decoder(nembed=args.nhid,
        dropout=args.dropout)
decoder_feture = models.DecoderFeature(nembed=args.nhid,
        dropout=args.dropout)
...
def train(epoch):
    ...
    if args.setting == 'recon_newG' or args.setting == 'recon' or args.setting == 'newG_cls':
        ...
        feature_out = decoder_feture(embed)
        print('feature_out:', feature_out)
        print('generated_G:', generated_G)
        loss_rec = utils.adj_mse_loss(generated_G[:ori_num, :][:, :ori_num], adj.detach().to_dense())
        # 保存生成的邻接矩阵和特征矩阵到本地
        save_path_generated_G = 'cora_adj_out.pt'
        save_path_feature_out = 'cora_feature_out.pt'
        torch.save(generated_G, save_path_generated_G)
        torch.save(feature_out, save_path_feature_out)
        print(f'Generated adjacency matrix saved to {save_path_generated_G}')
        print(f'Generated feature matrix saved to {save_path_feature_out}')
        save_path_label='cora_labels.pt'
        torch.save(labels_new, save_path_label)
        #ipdb.set_trace()
        # 打印形状和长度
        print('generated_G shape:', generated_G.shape)
        print('feature_out shape:', feature_out.shape)
