...
def get_parser():
    ...
    # parser.add_argument('--dataset', type=str, default='wiki-cs')
    # parser.add_argument('--dataset', type=str, default='BlogCatalog')
    # parser.add_argument('--dataset', type=str, default='citeseer')
    parser.add_argument('--size', type=int, default=100)

    parser.add_argument('--epochs', type=int, default=20100,
...
def print_class_acc(output, labels, class_num_list, pre='valid'):
    ...
    if labels.max() > 1:
        ...
        output_cpu = F.softmax(output, dim=-1).cpu().detach().numpy()
        auc_score = roc_auc_score(labels_cpu, output_cpu, average='macro', multi_class='ovr')
