                ...
                for step, (batch_x, batch_y) in enumerate(tqdm(data_loader)):
                    ...
                    if self.metrics_dict is not None:
                        for metric in self.metrics_dict.values():
                            if isinstance(metric, LogitsBasedMetric) or isinstance(metric, Metric) :
...
class CommonTorchTrainModel(TorchTrainModel):
    ...
    def cl_forward_and_compute_loss(self, batch_x, batch_y, batch_train_mask,epoch):
        ...
        _,indices = torch.sort(ce_loss,descending=False)
        # #num_large_losses = int(len(ce_loss)*(1 - epoch / 500 * 0.5))
        size = training_scheduler(0.5, epoch, 250,scheduler='linear')
        num_large_losses = int(len(ce_loss) * size)
        #num_large_losses = int(len(ce_loss)*(0.5+epoch / 500 * 0.5))
