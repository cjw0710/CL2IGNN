...
from rphgnn.layers.rphgnn_pre import rphgnn_propagate_and_collect, rphgnn_propagate_and_collect_label, rphgnn_propagate_and_collect_cora
# import umap
...
use_input_features = args.use_input # True
...
if running_leaderboard_mag:
    ...
    num_views = 3
    cl_rate = 0.6
    model_save_dir = "./RpHGNN/curri_model"
    if not os.path.exists(model_save_dir):
        os.makedirs(model_save_dir)
    model_save_path = os.path.join(model_save_dir, "curriculum_mag_seed_{}.pt".format(seed))
else:
...
if args.dataset == "cora":
    y = torch.load("./data/cora_labels.pt", map_location=torch.device('cpu')).detach().cpu().numpy()
elif args.dataset == "BlogCatalog":
    y = torch.load("./data/BlogCatalog_labels.pt", map_location=torch.device('cpu')).detach().cpu().numpy()
elif args.dataset == "citeseer":
    y = torch.load("./data/citeseer_labels_new.pt", map_location=torch.device('cpu')).detach().cpu().numpy()
elif args.dataset == "wiki":
    y = torch.load("./data/wiki-cs_labels.pt", map_location=torch.device('cpu')).detach().cpu().numpy()
else:
...
rphgnn_propagate_and_collect
feat_target_h_list_list, target_sorted_keys = rphgnn_propagate_and_collect_cora(hetero_graph, 
                        squash_k, 
                        inner_k, 
                        0.0,
                        target_node_type, 
...
if dataset in ["oag_L1", "oag_venue"]:
    ...
    metrics_dict = {
        ...
        #"micro_f1": torchmetrics.F1Score(task="multilabel", num_labels=int(num_classes), average="macro") if multi_label else torchmetrics.F1Score(task="multiclass", num_classes=int(num_classes), average="micro"),
        "macro_auc": torchmetrics.AUROC(task="multilabel", num_labels=int(num_classes), average="macro") if multi_label else torchmetrics.AUROC(task="multiclass", num_classes=int(num_classes), average="macro"),
...
def train_and_eval():
    ...
    if dataset in ["oag_L1", "oag_venue"]:
        ...
        early_stop_strategy = "score"
        early_stop_metric_names = ["macro_auc", "macro_f1"]
    ...
    if running_leaderboard_mag:
        ...
        with torch.no_grad():
            ...
            with torch.no_grad():
                with torch.autocast(device_type=device, dtype=torch.float32):
                    batch_y_pred_list = []
                    batch_logits_list = []
                    for step, (batch_x, batch_y,_) in enumerate(tqdm(train_data_loader)):
        ...
        with torch.no_grad():
            with torch.autocast(device_type=device): #dtype=self.autocast_dtype
                batch_y_pred_list = []
                batch_y_list = []
                #for step, (batch_x, batch_y) in enumerate(tqdm(test_data_loader)):
                batch_logits = model.predict(test_data_loader)
                # batch_y_pred = (torch.sigmoid(batch_logits) > 0.5).float()
                batch_y_pred = torch.argmax(batch_logits, dim=-1)
                batch_y = torch_y[test_index]
                # print(batch_y)
                # print(batch_y_pred)
                # exit()
        ...
        with torch.no_grad():        
            for metric_name, metric in metrics_dict.items():
                logs["{}".format(metric_name)] = metric.compute().item()
                metric.reset()
